...
extension CuriosityViewController {
    ...
    func getCuriosityPhotos() {
        NetworkManager().fetchCuriosityPhotos(page: pageCounter) { [weak self] photoResponse in
...
extension CuriosityViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: K.curiosityCell.rawValue, for: indexPath) as? CuriosityCollectionViewCell else { return UICollectionViewCell() }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let photoArray = photoArray {
            if indexPath.row == photoArray.count - 1 {
                // there are 35 pages, therefore i coded this way
                if pageCounter < 35 {
                    pageCounter += 1
                    getNewCuriosityPhotos()
...
extension CuriosityViewController {
    ...
    @IBAction func rhazButton(_ sender: UIButton) {
    ...
    @IBAction func fhazButton(_ sender: UIButton) {
        cameraName = sender.titleLabel?.text ?? ""
    ...
    @IBAction func mastButton(_ sender: UIButton) {
        cameraName = sender.titleLabel?.text ?? ""
